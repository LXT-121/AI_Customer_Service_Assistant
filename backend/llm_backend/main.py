...
# 创建 FastAPI 应用实例
app = FastAPI(title="Knowledge Desk API")
...
def _build_unavailable_message(feature_name: str) -> str:
    return (
        f"{feature_name} is unavailable in the current environment. "
        "Please install and verify the optional GraphRAG/LangGraph dependencies before using this endpoint."
    )
...
def _get_indexing_service():
    try:
        ...
        logger.error("Indexing service is unavailable", exc_info=True)
        raise RuntimeError(_build_unavailable_message("File indexing")) from exc
    return IndexingService()


def _get_langgraph_runtime() -> Dict[str, Any]:
    try:
        ...
        logger.error("LangGraph runtime is unavailable", exc_info=True)
        raise RuntimeError(_build_unavailable_message("LangGraph query")) from exc
...
def _get_rag_chat_service():
    try:
        from app.services.rag_chat_service import RAGChatService
    except Exception as exc:
        logger.error("RAG chat service is unavailable", exc_info=True)
        raise RuntimeError(_build_unavailable_message("RAG chat")) from exc
...
async def rag_chat_endpoint(request: RAGChatRequest):
    ...
    try:
        ...
        rag_chat_service = _get_rag_chat_service()
...
async def langgraph_query(
    ...
    try:
        ...
        runtime = _get_langgraph_runtime()
        InputState = runtime["InputState"]
        new_uuid = runtime["new_uuid"]
        graph = runtime["graph"]
